...
interface ApiCafe {
  ...
  amenities?: string[];
  tags?: string[];
  reviews?: Array<{ wifiQuality: number; powerOutlets: number; comfortLevel: number; comment: string; date: string; user: string }>;
  menu?: Array<{ name: string; category: string; price_idr: number; specialty: boolean }>;
  promo?: Array<{ title: string; description: string; valid_until: string; icon: string }>;
  location?: string;
  rating?: number;
}
...
const CafeList: React.FC = () => {
  ...
  const [deleteTarget, setDeleteTarget] = useState<{ id?: string; isBulk: boolean }>({ isBulk: false });
  const [deleteMessage, setDeleteMessage] = useState({ title: "", message: "" });
      ...
      try {
        ...
        if (result.success) {
          const mappedCafes: Cafe[] = result.data.map((apiCafe: ApiCafe) => ({
            ...
            openingHours: apiCafe.openingHours || { Weekdays: "", Weekends: "" },
        ...
        } else throw new Error(result.message || "Failed to fetch cafes");
      } catch (error) {
        console.error("Error fetching data:", error);
        toast.error("Failed to load cafes: " + (error instanceof Error ? error.message : "Unknown error"));
        setCafes([]);
        setAreas(["All Areas"]);
      } finally {
        setLoading(false);
  ...
  const handleBulkDelete = () => {
    ...
    setDeleteMessage({
      title: `Delete ${selectedCafes.length} Cafe${selectedCafes.length > 1 ? "s" : ""}`,
      message: `Are you sure you want to delete ${selectedCafes.length} cafe${selectedCafes.length > 1 ? "s" : ""}? This action cannot be undone.`,
    });
    setIsDeleteModalOpen(true);
  };
    ...
    try {
      if (deleteTarget.isBulk) {
        ...
        if (result.success) {
          ...
          setSelectedCafes([]);
          toast.success("Cafes deleted successfully");
        } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Cafes</h1>
        ...
        {selectedCafes.length > 0 && (
          <div className="bg-blue-50 px-4 py-2 border-b border-gray-200 flex items-center justify-between">
            <span className="text-sm text-blue-700">
              {selectedCafes.length} {selectedCafes.length === 1 ? "cafe" : "cafes"} selected
            </span>
            <Button
              variant="secondary"
              size="sm"
              ...
              <tr>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-10">
                  <input
                    ...
                    checked={currentItems.length > 0 && selectedCafes.length === currentItems.length}
                    onChange={handleSelectAll}
                  />
                </th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  <button className="flex items-center space-x-1 focus:outline-none" onClick={() => toggleSortOrder("name")}>
                    <span>Cafe</span>
                    {sortBy === "name" && <span className="text-gray-400">{sortOrder === "asc" ? "↑" : "↓"}</span>}
                  </button>
                </th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Location
                ...
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  <button className="flex items-center space-x-1 focus:outline-none" onClick={() => toggleSortOrder("rating")}>
                    <span>Rating</span>
                    {sortBy === "rating" && <span className="text-gray-400">{sortOrder === "asc" ? "↑" : "↓"}</span>}
                  </button>
                </th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  <button className="flex items-center space-x-1 focus:outline-none" onClick={() => toggleSortOrder("reviews")}>
                    <span>Reviews</span>
                    {sortBy === "reviews" && <span className="text-gray-400">{sortOrder === "asc" ? "↑" : "↓"}</span>}
                  </button>
                </th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Status
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-10 text-center text-gray-500">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="text-lg font-medium text-gray-600 mb-1">No cafes found</p>
                      <p className="text-sm text-gray-500">
                        {searchTerm || selectedArea ? "Try adjusting your search or filters" : "Add your first cafe to get started"}
                      </p>
                      {!searchTerm && !selectedArea && (
                        <Link to="/admin/add-cafe" className="mt-4">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="flex-shrink-0 h-10 w-10 rounded overflow-hidden">
                          <img
                            src={cafe.imageUrl || "https://via.placeholder.com/40x40?text=Cafe"}
                            alt={cafe.name}
                        ...
                        <div className="ml-4">
                          ...
                          <div className="text-xs text-gray-500 truncate max-w-xs">{cafe.address}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <div className="flex">
                          {[...Array(5)].map((_, i) => (
                            <Star
                              key={i}
                              size={14}
                              className={i < Math.round(cafe.rating) ? "text-amber-400 fill-amber-400" : "text-gray-300"}
                            />
                          ))}
                        </div>
                        <span className="ml-1 text-sm text-gray-500">{cafe.rating > 0 ? cafe.rating.toFixed(1) : "N/A"}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{cafe.reviews?.length || 0}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <Badge variant={cafe.featured ? "success" : "secondary"} rounded>
                        {cafe.featured ? "Featured" : "Regular"}
                      </Badge>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex items-center justify-end space-x-2">
                        <Link to={`/admin/cafes/${cafe.id}`} className="text-blue-600 hover:text-blue-900 p-1 rounded-full hover:bg-blue-50">
                        ...
                        <Link to={`/admin/cafes/${cafe.id}/edit`} className="text-indigo-600 hover:text-indigo-900 p-1 rounded-full hover:bg-indigo-50">
            ...
            <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-gray-700">
                  ...
                  <span className="font-medium">{indexOfLastItem > filteredCafes.length ? filteredCafes.length : indexOfLastItem}</span> of{" "}
                  <span className="font-medium">{filteredCafes.length}</span> results
                </p>
              </div>
              <div>
